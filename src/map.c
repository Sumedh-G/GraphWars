...
void InitTile(Tile *tile, const char *filepath, Frame frame_size) {
  if (tile == NULL || filepath == NULL) {
    WriteLog(LOG_ERROR,
             "Cannot initialize tile from NULL pointer. Recieved tile: %p and "
             "path: \"%s\"",
             tile, filepath);
    return;
  }
...
void InitTileSet(TileSet *tileset, const char *directorypath, Frame frame_size,
                 bool is_solid, bool is_visible) {
  if (!tileset || !directorypath) {
    WriteLog(LOG_ERROR,
             "Cannot initialize NULL tileset. Recieved tileset: %p, "
             "directorypath: \"%s\".",
             tileset, directorypath);
    return;
  }
  ...
  while (FileExists(tile_sprites.paths[file_index])) {
    ...
    if (file_index >= TILESET_CAPACITY) {
      WriteLog(LOG_WARNING,
               "Too many tiles in directory, cannot load more than %u tiles "
               "per tileset.",
               TILESET_CAPACITY);
      break;
    }

    InitTile(tileset->types + file_index, tile_sprites.paths[file_index],
             frame_size);
    file_index++;
  }
